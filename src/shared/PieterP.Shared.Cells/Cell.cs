...
namespace PieterP.Shared.Cells {
    ...
    {
        public static Cell<T> Create<T>( T initialValue = default( T ), Action? onValueChanged = null)
        {
            var cell = new ConcreteCell<T>( initialValue );
            if (onValueChanged != null)
                cell.ValueChanged += onValueChanged;
            return cell;
        }
        ...
        public static Cell<R> Derived<T1, T2, T3, T4, T5, T6, R>(Cell<T1> c1, Cell<T2> c2, Cell<T3> c3, Cell<T4> c4, Cell<T5> c5, Cell<T6> c6, Func<T1, T2, T3, T4, T5, T6, R> function) {
            var derived = new Derived<R>(() => function(c1.Value, c2.Value, c3.Value, c4.Value, c5.Value, c6.Value));
